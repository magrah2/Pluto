...
md"""
# Lano přes překážku – sjezd

Interaktivní model rovnováhy závaží na laně kotveném ve stromě.
Závaží visí na karabině **B**, lano vede ze stromu **A** přes karabinu k osobám **C₁**, **C₂**.
"""

# ╔═╡ aa000003-0000-0000-0000-000000000000
md"### Vstupní parametry"

# ╔═╡ aa000004-0000-0000-0000-000000000000
md"**Výška kotvení na stromě** h\_kotva [m]: $(@bind h_kotva PlutoUI.Slider(1.0:0.5:10.0, default=5.0, show_value=true))"

# ╔═╡ aa000005-0000-0000-0000-000000000000
md"**Vodorovná vzdálenost lidí od stromu** d\_strom [m]: $(@bind d_strom PlutoUI.Slider(2.0:0.5:20.0, default=8.0, show_value=true))"

# ╔═╡ aa000006-0000-0000-0000-000000000000
md"**Vzdálenost mezi lidmi** d\_lide [m]: $(@bind d_lide PlutoUI.Slider(1.0:0.5:10.0, default=4.0, show_value=true))"

# ╔═╡ aa000007-0000-0000-0000-000000000000
md"**Hmotnost závaží** m [kg]: $(@bind m_kg PlutoUI.Slider(10.0:5.0:150.0, default=50.0, show_value=true))"

# ╔═╡ aa000008-0000-0000-0000-000000000000
md"**Přebytek délky lana nad přímou A→C** [m]: $(@bind prebytek_L PlutoUI.Slider(0.1:0.1:5.0, default=0.7, show_value=true))"
...
begin
    ...
    T = (m_kg * g) / (2*uBA[3] + uBC1[3] + uBC2[3])
    ...
    uC1B   = (B - C1) / norm(B - C1)
    T_z_C  = T * uC1B[3]
    T_xy_C = T * norm(uC1B[1:2])
    β_C    = rad2deg(atan(T_z_C, T_xy_C))
...
| Veličina | Hodnota |
|:---|---:|
| L\_min (přímé lano A→C) | $(round(L_min, digits=2)) m |
| L (délka lana) | $(round(L, digits=2)) m |
| Poloha karabiny B | x = $(round(B[1], digits=2)) m,  z = $(round(B[3], digits=2)) m |
| Tah v každém laně T | $(round(T, digits=1)) N  (≈ $(round(T/g, digits=1)) kg) |
| Průvěs (pokles B pod přímou) | $(round(provis, digits=2)) m |
| Úhel A→B od horizontály | $(round(rad2deg(asin(abs(uBA[3]))),  digits=1))° |
| Úhel B→C od horizontály | $(round(rad2deg(asin(abs(uBC1[3]))), digits=1))° |
| Min. hmotnost osoby na konci | $(round(m_osoba, digits=1)) kg  (μ = $μ) |
"""

# ╔═╡ aa000011-0000-0000-0000-000000000000
function plot_geometry(A, B, C1, C2, Cmid, T, m, g, provis)
    uBA_v  = (A  - B) / norm(A  - B)
    ...
    mid_AB  = (A[1:2]  + B[1:2])  / 2
    mid_BC1 = (B[1:2]  + C1[1:2]) / 2
    mid_BC2 = (B[1:2]  + C2[1:2]) / 2
    text!(ax1, mid_AB[1]-0.5,  mid_AB[2]+0.1;  text="|AB|=$(round(norm(A-B),  digits=2)) m", fontsize=10, color=:royalblue)
    text!(ax1, mid_BC1[1]+0.1, mid_BC1[2]+0.1; text="|BC₁|=$(round(norm(B-C1),digits=2)) m", fontsize=10, color=:royalblue, align=(:left, :center))
    text!(ax1, mid_BC2[1]+0.1, mid_BC2[2]-0.1; text="|BC₂|=$(round(norm(B-C2),digits=2)) m", fontsize=10, color=:tomato,    align=(:left, :center))
    ...
    mid_AB_xz = ([A[1], A[3]] + [B[1], B[3]]) / 2
    mid_BC_xz = ([B[1], B[3]] + [C1[1], C1[3]]) / 2
    text!(ax2, mid_AB_xz[1]-0.3, mid_AB_xz[2];
          text="|AB|=$(round(norm(A-B),  digits=2)) m", fontsize=10, color=:royalblue, align=(:right, :center))
    text!(ax2, mid_BC_xz[1]+0.1, mid_BC_xz[2]+0.15;
...
plot_geometry(A, B, C1, C2, Cmid, T, m_kg, g, provis)
...
manifest_format = "2.0"
project_hash = "370222de4eec51158adef7dca8392fa3b9b0e32e"
